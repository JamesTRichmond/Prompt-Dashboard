...
The output is not a prompt. It is a being with a perspective. The being is the product.

## What it is

A living soundboard. Knobs, buttons, a panel that argues. You turn dials — era, class, reverence, temperature, gender, political aspiration, density, something you cannot name — and something is born. Not a refined version of your input. A transmogrification. Fire plus coal is not fire anymore. It is a compound.

The dials are not numbers. Each is an agent with a bias, a voice, an agenda. They do not set values. They argue with your input from their corner of the world. The output is the negotiation between them.
...
It is a self-model, a predictor, and a live diagnostic, all at once.

- Something changes — a breakup, a raise, a medication shift, winter coming, seasonal affective disorder — and the model notices the collision before you feel it.
- It offers a perspective shift as preemptive care. Not a doctor. A mirror that warns you the weather is turning.
- You can tweak your own dials — "what if I got a dog," "what if I slept differently" — and watch the model predict who you would become. A simulation of yourself, run forward, so you can see the consequences of a change before you make it.

The life-context layer is what makes it stop being a toy. The machine knows you had a bad week. It does not offer prompts. It offers the right combination for where you are.
...
- Dials are agents, not sliders. Each carries a bias, a voice, an agenda.
- Combination is the operation. Transmogrify, not optimize. Mutate, evolve, time travel.
- Life context weights the biases. The self-model is the continuous subject.
- Evolution happens when agents breed or mutate across sessions.
- Mixture-of-experts panels are born fresh every time. No static panel. No fixed routing.
- The gap between what you can model and what emerges is the feature, not the bug.

## Externals

A soundboard. Knobs, buttons, a living panel. You turn, you press, something is born. No menus, no settings pages, no "optimize your prompt" copy. Just the thrill of combination and the wonder of what comes out.

## Privacy and the self-model

The self-model requires trust. It lives on the user's device or under explicit consent. It never ships raw life data anywhere without the user's knowledge. The first public version stays static and browser-only. The self-model is the horizon, not the launch.
